...
template <typename Base, typename T, typename U, typename = typename std::enable_if_t<! std::is_same<T, bool>::value>>
Expr<Base, T> operator==(const Nvp<Base, T>& lhs, const U& rhs) {
    return Expr<Base, T>(lhs, rhs);
}

template <typename Base, typename T, typename = typename std::enable_if_t<std::is_same<T, bool>::value>>
